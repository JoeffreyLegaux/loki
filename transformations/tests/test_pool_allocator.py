...
contains
    ...
    if nclv_param:
        if frontend == OMNI:
            ...
            trafo_data_compare = f'3 * max(c_sizeof(real(1, kind={kind_real})), 8) * klon + max(c_sizeof(real(1, kind={kind_real})), 8) * klev * klon'

            if generate_driver_stack:
                stack_size = f'3 * max(c_sizeof(real(1, kind={kind_real})), 8) * nlon / max(c_sizeof(real(1, kind=jprb)), 8)'
                stack_size += f'+ max(c_sizeof(real(1, kind={kind_real})), 8) * nlon * nz / max(c_sizeof(real(1, kind=jprb)), 8)'
            else:
                stack_size = f'3 * max(c_sizeof(real(1, kind={kind_real})), 8) * nlon / max(c_sizeof(real(1, kind={kind_real})), 8)'
                stack_size += f'+ max(c_sizeof(real(1, kind={kind_real})), 8) * nlon * nz / max(c_sizeof(real(1, kind={kind_real})), 8)'
        else:
            # pylint: disable-next=line-too-long
            trafo_data_compare = f'max(c_sizeof(real(1, kind={kind_real})), 8) * klon + max(c_sizeof(real(1, kind={kind_real})), 8) * klev * klon'
            ...
            stack_size += f'+ max(c_sizeof(real(1, kind={kind_real})), 8) * nlon * nz / max(c_sizeof(real(1, kind=jprb)), 8)'
            stack_size += f'+ max(c_sizeof(real(1, kind={kind_real})), 8) * nclv * nlon / max(c_sizeof(real(1, kind=jprb)), 8)'

    else:
        # pylint: disable-next=line-too-long
        trafo_data_compare = f'max(c_sizeof(real(1, kind={kind_real})), 8) * klon + max(c_sizeof(real(1, kind={kind_real})), 8) * klev * klon'
        # pylint: disable-next=line-too-long
        trafo_data_compare += f'+ max(c_sizeof(real(1, kind={kind_real})), 8) * nclv + max(c_sizeof(real(1, kind={kind_real})), 8) * klon * nclv'

        if generate_driver_stack:
            stack_size = f'3 * max(c_sizeof(real(1, kind={kind_real})), 8) * nlon / max(c_sizeof(real(1, kind=jprb)), 8)'
            stack_size += f'+ max(c_sizeof(real(1, kind={kind_real})), 8) * nlon * nz / max(c_sizeof(real(1, kind=jprb)), 8)'
            stack_size += f'+ 2 * max(c_sizeof(real(1, kind={kind_real})), 8) / max(c_sizeof(real(1, kind=jprb)), 8)'
        else:
            stack_size = f'3 * max(c_sizeof(real(1, kind={kind_real})), 8) * nlon / max(c_sizeof(real(1, kind={kind_real})), 8)'
            stack_size += f'+ max(c_sizeof(real(1, kind={kind_real})), 8) * nlon * nz / max(c_sizeof(real(1, kind={kind_real})), 8)'
            stack_size += f'+ 2 * max(c_sizeof(real(1, kind={kind_real})), 8) / max(c_sizeof(real(1, kind={kind_real})), 8)'
...
contains
    ...
    stack_size = f'max(max(c_sizeof(real(1, kind={kind_real})), 8)*nlon + max(c_sizeof(real(1, kind={kind_real})), 8)*nlon*nz + '
    stack_size += f'2*max(c_sizeof(int(1, kind={kind_int})), 8)*nlon + max(c_sizeof(logical(true, kind={kind_log})), 8)*nz,'
    stack_size += f'3*max(c_sizeof(real(1, kind={kind_real})), 8)*nz*nlon)/max(c_sizeof(real(1, kind=jprb)), 8)'
...
stack-size = "4*{stack_size}"
    ...
    if stack_size_config == "file":
        scheduler = Scheduler(paths=[basedir], config=SchedulerConfig.from_file(basedir / 'loki_config.config'), frontend=frontend,
                              definitions=definitions)
    else:
        scheduler = Scheduler(paths=[basedir], config=SchedulerConfig.from_dict(config), frontend=frontend,
                              definitions=definitions)
...
@pytest.mark.parametrize('frontend', available_frontends())
def test_pool_allocator_args_vs_kwargs(frontend, block_dim, caplog):
    fcode_driver = """
subroutine driver(NLON, NZ, NB, FIELD1, FIELD2)
    use kernel_mod, only: kernel, kernel2
    use parkind1, only : jpim
